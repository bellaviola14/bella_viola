              ...
              <div id="content_layout">
				...
				<div class="card">
                                    <div class="card-header border-bottom">
                                        <a class="btn btn-outline-primary btn-sm" href="{{ route ('mahasiswa.tambah') }}">
                          ...
                          <div class="overflow-hidden ">
                            <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700 data-table">
                              ...
                              <tbody class="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
								@forelse ($mahasiswa as $tampiluser) 
                                <tr>
                                  <td class="table-td">{{ $loop->iteration }}</td>
                                  <td class="table-td ">{{$tampiluser->nim}}</td>
                                  <td class="table-td ">{{$tampiluser->nama_mahasiswa}}</td>
                                  <td class="table-td ">{{$tampiluser->jalur_masuk}}</td>
                                  <td class="table-td ">{{$tampiluser->foreign_kelas->nama_kelas}} ({{$tampiluser->foreign_kelas->angkatan}})</td>
                                  <td class="table-td ">
                                    <div class="flex space-x-3 rtl:space-x-reverse">
                                      <a class="action-btn" href="{{ route ('mahasiswa.edit', $tampiluser->id) }}" target="_blank">
...
<form action="{{ route('mahasiswa.hapus', $tampiluser->id) }}" method="post">
                                                                                                                            @csrf
                                                                                                                            @method('DELETE')									  
