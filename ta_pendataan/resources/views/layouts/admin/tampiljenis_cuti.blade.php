              ...
              <div id="content_layout">
				...
				<div class="card">
                                    <div class="card-header border-bottom">
                                        <a class="btn btn-outline-primary btn-sm" href="{{ route ('jenis_cuti.tambah') }}">
                            ...
                            <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700 data-table">
                              ...
                              <tbody class="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                                ...
                                <tr>
                                  ...
                                  <td class="table-td ">{{$tampildata->jenis_cuti}}</td>
                                  <td class="table-td ">{{$tampildata->created_at}}</td>
                                  <td class="table-td ">{{$tampildata->updated_at}}</td>
                                  <td class="table-td ">
                                    <div class="flex space-x-3 rtl:space-x-reverse">
                                      <a class="action-btn" href="{{ route ('jenis_cuti.edit', $tampildata->id) }}" target="_blank">
                                        <iconify-icon icon="heroicons:pencil-square"></iconify-icon>
                                      </a>
                                      <a class="action-btn" href="javascript:void(0);" data-bs-toggle="modal" data-bs-target="#modalhapus{{ $tampildata->id }}">
                                        <iconify-icon icon="heroicons:trash"></iconify-icon>
                                      </a>
<form action="{{ route('jenis_cuti.hapus', $tampildata->id) }}" method="post">
                                                                                                                            @csrf
                                                                                                                            @method('DELETE')									  
											...
											<div class="relative bg-white rounded-lg shadow dark:bg-slate-700">
											  ...
											  <div class="flex items-center justify-between p-5 border-b rounded-t dark:border-slate-600 bg-danger-500">
												<h3 class="text-base font-medium text-white dark:text-white capitalize">
												  Hapus Data {{ $tampildata->jenis_cuti }}?
												</h3>
