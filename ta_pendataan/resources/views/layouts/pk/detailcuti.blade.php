                  ...
                  <div class="space-y-5 divide-y divide-slate-100 dark:divide-slate-700 -mx-6">
                    ...
                    <div class="pt-4 px-6">
                      <h5 class="text-slate-600 dark:text-slate-300 text-xl font-medium mb-4">
						...
						@if ($cuti->status == 'diajukan ke pengadministrasi kepegawaian')
						  <form action="{{ route('pk.teruskancuti', $cuti->id) }}" method="post">
							@csrf
							@method('PUT')
							<button type="submit" class="btn btn-primary">Teruskan ke Kasubbag Umum</button>
						  </form>						
						@elseif ($cuti->status == 'serahkan ke pengadministrasi kepegawaian')
						  <form action="{{ route('pk.teruskanv2', $cuti->id) }}" method="post">
							@csrf
							@method('PUT')
							<button type="submit" class="btn btn-primary">Serahkan Ke <br> Pengadministrasi Umum</button>
						  </form						
						@endif
