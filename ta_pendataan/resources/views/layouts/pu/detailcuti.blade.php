                ...
                <div class="card-body p-6">
                  ...
                  <div class="space-y-5 divide-y divide-slate-100 dark:divide-slate-700 -mx-6">
                    ...
                    <div class="pt-4 px-6">
					  ...
					  @if ($cuti->status == 'serahkan ke pengadministrasi umum')
					  
                      <form action="{{ route('pu.teruskancuti', $cuti->id) }}" method="post">
                        @csrf
                        @method('PUT')
						
                          <div class="input-area relative pl-28">
                            <label for="largeInput" class="inline-inputLabel">No Surat</label>
                            <input type="text" class="form-control" required name="no_surat_cuti">
                          </div>	

						<br>						  
						
                        <button type="submit" class="btn btn-primary">Proses Ke Pengadministrasi Kepegawaian</button>
						
						@elseif ($cuti->status == 'proses ke bagian pengadministrasi umum')
						
                      <form action="{{ route('pu.terima', $cuti->id) }}" method="post">
                        @csrf
                        @method('PUT')						      

						<br>						  
						
                        <button type="submit" class="btn btn-primary">Teruskan Ke Pegawai/Diterima</button>						
						
						@endif

                      </form>
                     <!-- <a class="btn btn-danger" href="javascript:void(0);" data-bs-toggle="modal" data-bs-target="#modaltolak">Tolak</a> !-->
